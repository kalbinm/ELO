(function () {
  ...
  function checkQuietVoice() {
    ...
    if (quietBusy && quietRecorder && quietRecorder.state === 'recording') { if (rms > 0.014) quietLastVoice = now; if (now - quietLastVoice > 1200 || now - quietStarted > 5000) stopQuietChunk(); }
  ...
  function stripJarvis(text) { return normalize(text).replace(/^(hey|hay|hello|hi)\s+jarvis[\s,!.:-]*/i, '').trim(); }
  function routeCommand(text) { var command = stripJarvis(text); if (!command) return speak('Yes, sir. I am here.'); if (/\b(what(?:'s| is)? the )?time\b/i.test(command)) return tellTime(); if (isNews(command)) return fetchBrief('/api/news', command); if (isSearch(command)) return fetchBrief('/api/search', command); sendChat(command); }
  ...
  function sendChat(text) {
    ...
    fetch('/api/chat', { method: 'POST', headers: headers, body: JSON.stringify({ messages: messages }) }).then(function (r) { return r.json().then(function (d) { return { ok: r.ok, data: d, status: r.status }; }); }).then(function (result) { if (!result.ok && result.status === 401) { var code = window.prompt('Enter the JARVIS access code:'); if (code) { window.localStorage.setItem('elo_access_code', code); return sendChat(text); } throw new Error('Access code required.'); } if (!result.ok) throw new Error(result.data.error || 'JARVIS is unavailable.'); var reply = String(result.data.reply || '').trim() || 'I did not catch that. Please press the reactor and try again.'; messages.push({ role: 'assistant', content: reply }); if (messages.length > 6) messages.shift(); answer.textContent = reply; speak(reply); }).catch(function (error) { answer.textContent = 'I’m here, but my brain is having a tiny coffee break.'; setState('idle', 'Press to speak'); setStatus(error.message); setActivity('Press reactor to talk', false); });
  }
  function preferredVoice(voices) { var preferred = ['Google UK English Male', 'Microsoft George', 'Daniel', 'Google UK English Female', 'Google US English', 'Microsoft David', 'Samantha', 'Microsoft Zira'], i, j; for (i = 0; i < preferred.length; i++) for (j = 0; j < voices.length; j++) if (voices[j].name === preferred[i]) return voices[j]; for (j = 0; j < voices.length; j++) if (/^en(-|_)GB/i.test(voices[j].lang) && /male|man|daniel|george|arthur/i.test(voices[j].name)) return voices[j]; for (j = 0; j < voices.length; j++) if (/^en(-|_)GB/i.test(voices[j].lang)) return voices[j]; for (j = 0; j < voices.length; j++) if (/^en(-|_)/i.test(voices[j].lang) && voices[j].localService) return voices[j]; for (j = 0; j < voices.length; j++) if (/^en(-|_)/i.test(voices[j].lang)) return voices[j]; return null; }
  function speechText(text) { return String(text || '').replace(/\s+/g, ' ').replace(/^(JARVIS\s*[:,-]\s*)/i, '').trim(); }
  function speak(text) { if (!window.speechSynthesis || !window.SpeechSynthesisUtterance) { setState('idle', 'Press to speak'); return; } setState('speaking', 'Speaking...'); setActivity('JARVIS is speaking', true); var utterance = new SpeechSynthesisUtterance(speechText(text)), voices = window.speechSynthesis.getVoices(); utterance.lang = 'en-GB'; utterance.voice = preferredVoice(voices); utterance.volume = 1; utterance.rate = 0.88; utterance.pitch = 0.9; utterance.onend = function () { setState('idle', 'Press to speak'); setActivity('Press reactor to talk', false); resetIdle(); }; utterance.onerror = utterance.onend; window.speechSynthesis.cancel(); window.speechSynthesis.speak(utterance); }
