(function () {
  ...
  function startQuietListening() {
    ...
    navigator.mediaDevices.getUserMedia(constraints).catch(function () { return navigator.mediaDevices.getUserMedia({ audio: true }); }).then(function (stream) {
      ...
      quietAudioContext.createMediaStreamSource(stream).connect(quietAnalyser); systemState.textContent = 'VOICE LINK ACTIVE'; setState('listening', 'Say “Hey ELO”'); setStatus('Microphone on — quiet voice mode'); setActivity('Quiet speech monitor', true); startQuietVad();
  ...
  function armListening() { armed = true; setActivity('Wake word standby', true); setState('listening', 'Say “Hey ELO”'); if (quietSupported) startQuietListening(); else startListening(); }
  ...
  window.addEventListener('beforeunload', function () { stopSpeech(); stopQuietListening(); }); setupRecognition(); resetIdle(); updateTopClock(); setInterval(updateTopClock, 30000); setStatus('Listening for “Hey ELO”...'); setTimeout(function () { if (quietSupported || recognition) armListening(); }, 300);
}());
