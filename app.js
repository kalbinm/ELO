(function () {
  ...
  function handleTranscript(text) {
    ...
    if (!spoken) return;
    if (!commandMode && wake) { commandMode = true; transcript = spoken.slice(wake.index + wake.length).trim(); setState('listening', 'I’m listening...'); setActivity('Wake word heard', true); stopSpeech(); if (transcript) finishCommand(transcript); else { setStatus('Go ahead.'); commandTimer = setTimeout(function () { commandMode = false; setState('listening', 'Say “Hey JARVIS”'); setActivity('Wake word standby', true); }, 7000); } }
  ...
  function startQuietListening() {
    ...
    navigator.mediaDevices.getUserMedia(constraints).catch(function () { return navigator.mediaDevices.getUserMedia({ audio: true }); }).then(function (stream) {
      ...
      quietAudioContext.createMediaStreamSource(stream).connect(quietAnalyser); systemState.textContent = 'VOICE LINK ACTIVE'; setState('listening', 'Say “Hey JARVIS”'); setStatus('Microphone on — quiet voice mode'); setActivity('Quiet speech monitor', true); startQuietVad();
  ...
  function setupRecognition() {
    ...
    recognition.onstart = function () { recognitionRunning = true; systemState.textContent = 'VOICE LINK ACTIVE'; setStatus('Microphone on — say “Hey JARVIS”'); setState('listening', armed ? 'Say “Hey JARVIS”' : 'Listening...'); setActivity('Wake word standby', true); };
  ...
  function armListening() { armed = true; setActivity('Wake word standby', true); setState('listening', 'Say “Hey JARVIS”'); if (quietSupported) startQuietListening(); else startListening(); }
  ...
  function fetchBrief(endpoint, text) { answer.textContent = 'Checking the latest information...'; fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: text }) }).then(function (r) { return r.json().then(function (d) { return { ok: r.ok, data: d }; }); }).then(function (result) { if (!result.ok) throw new Error(result.data.error || 'Search is unavailable.'); var reply = String(result.data.reply || '').trim(); answer.textContent = reply || 'I could not find a useful result.'; speak(reply || 'I could not find a useful result.'); }).catch(function (error) { answer.textContent = 'I could not reach the web right now.'; setState('idle', 'Say “Hey JARVIS”'); setStatus(error.message); setActivity('Wake word standby', true); }); }
  ...
  function speak(text) { if (!window.speechSynthesis) { setState('idle', 'Say “Hey JARVIS”'); return; } setState('speaking', 'Speaking...'); setActivity('JARVIS is speaking', true); var utterance = new SpeechSynthesisUtterance(text), voices = window.speechSynthesis.getVoices(); utterance.voice = voices.filter(function (voice) { return /^en(-|_)/i.test(voice.lang) && /Google|Samantha|Daniel|Microsoft/i.test(voice.name); })[0] || voices.filter(function (voice) { return /^en(-|_)/i.test(voice.lang); })[0] || null; utterance.volume = 1; utterance.rate = 0.93; utterance.pitch = 1.04; utterance.onend = function () { setState('listening', 'Say “Hey JARVIS”'); setActivity('Wake word standby', true); resetIdle(); if (armed && quietSupported) startQuietListening(); else if (armed) startListening(); }; utterance.onerror = utterance.onend; window.speechSynthesis.speak(utterance); }
  ...
  orb.addEventListener('click', function () { resetIdle(); if (window.speechSynthesis && window.speechSynthesis.speaking) { stopSpeech(); setState('listening', 'Say “Hey JARVIS”'); return; } armListening(); });
  ...
  window.addEventListener('beforeunload', function () { stopSpeech(); stopQuietListening(); }); setupRecognition(); resetIdle(); updateTopClock(); setInterval(updateTopClock, 30000); setStatus('Listening for “Hey JARVIS”...'); setTimeout(function () { if (quietSupported || recognition) armListening(); }, 300);
}());
