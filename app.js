(function () {
  ...
  function preferredVoice(voices) { var preferred = ['Google UK English Male', 'Google US English', 'Microsoft David', 'Daniel', 'Samantha', 'Google UK English Female', 'Microsoft Zira'], i, j; for (i = 0; i < preferred.length; i++) for (j = 0; j < voices.length; j++) if (voices[j].name === preferred[i]) return voices[j]; for (j = 0; j < voices.length; j++) if (/^en(-|_)/i.test(voices[j].lang) && voices[j].localService) return voices[j]; for (j = 0; j < voices.length; j++) if (/^en(-|_)/i.test(voices[j].lang)) return voices[j]; return null; }
  function speechText(text) { return String(text || '').replace(/\s+/g, ' ').replace(/^(JARVIS\s*[:,-]\s*)/i, '').trim(); }
  function speak(text) { if (!window.speechSynthesis || !window.SpeechSynthesisUtterance) { setState('idle', 'Press to speak'); return; } setState('speaking', 'Speaking...'); setActivity('JARVIS is speaking', true); var utterance = new SpeechSynthesisUtterance(speechText(text)), voices = window.speechSynthesis.getVoices(); utterance.voice = preferredVoice(voices); utterance.volume = 1; utterance.rate = 0.9; utterance.pitch = 0.96; utterance.onend = function () { setState('idle', 'Press to speak'); setActivity('Press reactor to talk', false); resetIdle(); }; utterance.onerror = utterance.onend; window.speechSynthesis.cancel(); window.speechSynthesis.speak(utterance); }
